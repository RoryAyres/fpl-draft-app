...
- **Architecture:** Single-page application (SPA) built with Tailwind CSS, vanilla JavaScript (modular object structure), and designed to run with a serverless backend proxy (Vercel) to bypass FPL API CORS restrictions. Features a graceful fallback to mock data when the live API/proxy is unreachable[cite: 2]. 
...
- **`State`**: Centralised state store managing static player metadata, live scores, team lineups, current gameweek, and active team ID[cite: 2]. 
...
- **`API`**: Manages data fetching through the Vercel proxy with automatic fallback error handling. Utilises cache-busting timestamp tokens (`&_t=`) on manual refresh to bypass aggressive mobile browser caching[cite: 2].
- **`Render`**: Dynamically constructs DOM elements for[cite: 2]:
...
To run this live without CORS issues, you must use this specific structure alongside `index.html`[cite: 2].
